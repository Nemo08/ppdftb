...
func TestCollectCadFiles(t *testing.T) {
	...
	t.Run("single file", func(t *testing.T) {
		files := CollectCadFiles(filepath.Join(dir, "drawing.dwg"), "")
		if len(files) != 1 {
	...
	t.Run("both source and dir merge results", func(t *testing.T) {
		files := CollectCadFiles(filepath.Join(dir, "drawing.dwg"), dir)
		if len(files) != 3 {
	...
	t.Run("no cad files in dir", func(t *testing.T) {
		...
		files := CollectCadFiles("", emptyDir)
		if len(files) != 0 {
	...
	t.Run("non-existent source", func(t *testing.T) {
		files := CollectCadFiles(filepath.Join(dir, "nonexistent.dwg"), "")
		if len(files) != 0 {
			t.Errorf("CollectCadFiles() = %d files, want 0", len(files))
		}
	})
